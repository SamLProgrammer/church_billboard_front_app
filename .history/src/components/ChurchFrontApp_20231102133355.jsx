...
export const ChurchFrontApp = () => {
  ...
  const callUseValidateLogin = async () => {
    const response1 = JSON.parse(JSON.stringify(await validateLogin(username, password)));
    const {sessionToken, error} = response1;
    if (error == null) {
      localStorage.setItem('sessionToken', sessionToken);
      const {availableMonths, error} = JSON.parse(JSON.stringify(await fetchMonths()));
