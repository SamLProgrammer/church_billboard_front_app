      ...
      case "C":
        {
          if (existingData[selectedMonth].cleaningList?.length > 0) {
            existingData[selectedMonth].cleaningList = existingData[
              ...
              const modifiedItem = transformedChangedRows.filter((row) => {row.familyEventDate == cleaningItem.familyEventDate});
              console.log((modifiedItem.length > 0 ) ? modifiedItem[0].family : 'nothing');
              return modifiedItem.length > 0 ? {...cleaningItem, family: modifiedItem[0].family} : cleaningItem;
            });
          } else {
